...
// Sum allocated qty and total SOL after fees with checked math
    let allocated_qty = bids.iter().try_fold(0u64, |acc, b| acc.checked_add(b.bid_qty))
        .ok_or(CustomError::Overflow)?;
    let total_sol_after_fees = bids.iter().try_fold(0u64, |acc, b| {
        b.bid_qty
    ...
    // H2: settlement “change” (overpayment refund) with checked math
    let clearing_price_tmp = if bids.is_empty() { 0 } else { bids.last().unwrap().bid_sol };
    let mut total_change: u64 = 0;
    for bid in bids.iter() {
        let paid = bid
            .bid_qty
            .checked_mul(bid.bid_sol)
            .and_then(|v| v.checked_sub(bid.bid_fee))
            .ok_or(CustomError::Overflow)?;
        let price_for_exact = if clearing_price_tmp == 0 { bid.bid_sol } else { clearing_price_tmp };
        let exact = bid
            .bid_qty
            .checked_mul(price_for_exact)
            .ok_or(CustomError::Overflow)?;
        let owed = paid.saturating_sub(exact);
        total_change = total_change.checked_add(owed).ok_or(CustomError::Overflow)?;
    }

    // Net SOL raised (before rent)
    let net_sol_raised_before_rent = total_sol_after_fees.saturating_sub(total_change);

    // Rent (0-byte account baseline)
    let rent = Rent::get()?; // avoid unwrap DoS
    let rent_exempt_minimum = rent.minimum_balance(0);

    // Final net SOL raised
    let net_sol_raised = net_sol_raised_before_rent.saturating_sub(rent_exempt_minimum);

    let supply_qty = auction
        .token_supply
        .saturating_div(10u64.pow(auction.token_decimals as u32));
    ...
    let clearing_price = match status {
        ...
        AuctionStatus::Live => {
            if bids.is_empty() { None } else { Some(bids.last().unwrap().bid_sol) }
        }
